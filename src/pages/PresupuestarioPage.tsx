...
const PresupuestarioPage = () => {
  ...
  const selectedMunicipalityData = useMemo(() => {
    ...
    return municipalities.filter(
      m => m.name === selectedMunicipality && selectedYears.includes(m.year)
  ...
  const allMunicipalities = Array.from(
    new Map(municipalities.map(m => [m.name, m])).values()
  ).sort((a, b) => (a.name || '').localeCompare(b.name || ''));

  const departments = Array.from(
    new Set(allMunicipalities.map(m => m.department))
  ).sort() as string[];

  const municipalitiesByDept = selectedDepartment
    ...
    return (
      <ResponsiveContainer width="100%" height={isMobile ? 250 : 300}>
        <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
          ...
          <YAxis />
          <Tooltip formatter={(value) => `L ${formatNumber(value, 0)}M`} />
          {!isMobile && <Legend />}
          ...
          <table className={`w-full ${isMobile ? 'text-xs' : 'text-sm'}`}>
            <thead className="bg-blue-600 text-white">
              <tr>
                ...
                <th className={`text-right font-semibold ${isMobile ? 'px-2 py-2' : 'px-4 py-3'}`}>Superávit/Déficit</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {selectedMunicipalityData.map((mun, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className={`text-right ${isMobile ? 'px-2 py-2' : 'px-4 py-3'}`}>{formatCurrency(mun.presupuesto_municipal)}</td>
                  <td className={`text-right ${isMobile ? 'px-2 py-2' : 'px-4 py-3'}`}>{formatCurrency(mun.total_egresos)}</td>
                  <td className={`text-right font-bold ${mun.superavit_deficit > 0 ? 'text-green-600' : 'text-red-600'} ${isMobile ? 'px-2 py-2' : 'px-4 py-3'}`}>
              ...
              {selectedMunicipalityData.map((mun, idx) => (
                <React.Fragment key={idx}>
                  <tr className="bg-blue-50 font-bold">
                    <td colSpan={2} className={`${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>Año {mun.year}</td>
                  </tr>
                  <tr>
                    <td className={`${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>Presupuesto</td>
                    <td className={`text-right ${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>{formatCurrency(mun.presupuesto_municipal)}</td>
                  </tr>
                  <tr className="bg-gray-50">
                    <td className={`${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>Ingresos Propios</td>
                    <td className={`text-right ${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>{formatCurrency(mun.ingresos_propios)}</td>
                  </tr>
                </React.Fragment>
              ))}
            </tbody>
              ...
              {selectedMunicipalityData.map((mun, idx) => (
                <React.Fragment key={idx}>
                  ...
                  <tr>
                    ...
                    <td className={`text-right ${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>{formatCurrency(mun.impuesto_bi)}</td>
                  </tr>
                </React.Fragment>
              ))}
            </tbody>
              ...
              {selectedMunicipalityData.map((mun, idx) => (
                <React.Fragment key={idx}>
                  ...
                  <tr>
                    ...
                    <td className={`text-right ${isMobile ? 'px-2 py-1' : 'px-3 py-1'}`}>{formatCurrency(mun.gastos_funcionamiento)}</td>
                  </tr>
                </React.Fragment>
              ))}
            </tbody>
  ...
  const renderCapitalTab = () => {
    ...
    return (
      <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : 'md:grid-cols-2'}`}>
        {selectedMunicipalityData.map((mun, idx) => (
          <div key={idx} className={`border-l-4 border-blue-600 p-4 bg-blue-50 rounded ${isMobile ? 'text-sm' : ''}`}>
            ...
            <p className={`font-bold text-blue-700 mt-1 ${isMobile ? 'text-xl' : 'text-2xl'}`}>{formatCurrency(mun.ingresos_capital)}</p>
          </div>
        ))}
      </div>
    );
  ...
  const renderCapitalDeudaTab = () => {
    ...
    return (
      <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : 'md:grid-cols-2'}`}>
        {selectedMunicipalityData.map((mun, idx) => (
          <div key={idx} className={`border-l-4 border-purple-600 p-4 bg-purple-50 rounded ${isMobile ? 'text-sm' : ''}`}>
            ...
            <p className={`font-bold text-purple-700 mt-1 ${isMobile ? 'text-xl' : 'text-2xl'}`}>{formatCurrency(mun.gastos_capital_deuda)}</p>
          </div>
        ))}
      </div>
    );
        ...
        <div className={`bg-white rounded-lg shadow-md overflow-y-auto max-h-96 ${isMobile ? 'p-3' : ''}`}>
          <table className={`w-full ${isMobile ? 'text-xs' : 'text-sm'}`}>
            <thead className="bg-blue-600 text-white">
              <tr>
                <th className={`text-left font-semibold ${isMobile ? 'px-2 py-2' : 'px-3 py-2'}`}>Año</th>
                <th className={`text-right font-semibold ${isMobile ? 'px-2 py-2' : 'px-3 py-2'}`}>Total Egresos</th>
                <th className={`text-right font-semibold ${isMobile ? 'px-2 py-2' : 'px-3 py-2'}`}>Superávit/Déficit</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {selectedMunicipalityData.map((mun, idx) => (
                <tr key={idx} className={mun.superavit_deficit > 0 ? 'bg-green-50' : 'bg-red-50'}>
                  ...
                  <td className={`text-right ${isMobile ? 'px-2 py-2' : 'px-3 py-2'}`}>{formatCurrency(mun.total_egresos)}</td>
                  <td className={`text-right font-bold ${mun.superavit_deficit > 0 ? 'text-green-600' : 'text-red-600'} ${isMobile ? 'px-2 py-2' : 'px-3 py-2'}`}>
                    {formatCurrency(mun.superavit_deficit)}
                  </td>
                </tr>
              ))}
  ...
  const renderKPICards = () => {
    ...
    return (
      <div className={`grid gap-4 ${isMobile ? 'grid-cols-2' : isTablet ? 'grid-cols-2' : 'grid-cols-4'}`}>
        {kpis.map((kpi, index) => (
          <div key={index} className={`border-l-4 rounded-lg p-4 ${kpi.color} shadow-sm hover:shadow-md transition`}>
  ...
  return (
    <DashboardLayout title="Datos Financieros">
      <div className="space-y-6">
        ...
        <div className={`bg-white rounded-lg shadow-md overflow-hidden ${isMobile ? 'p-3' : 'p-6'} space-y-4`}>
          ...
          {selectedDepartment && (
            <div>
              ...
              <div className={`grid gap-2 p-3 border border-gray-300 rounded-lg bg-gray-50 max-h-40 overflow-y-auto ${
                isMobile ? 'grid-cols-2' : 'grid-cols-3 lg:grid-cols-4'
              }`}>
                {municipalitiesByDept.map(mun => (
                  <label key={mun.name} className={`flex items-center space-x-2 cursor-pointer p-1 hover:bg-blue-100 rounded ${isMobile ? 'text-xs' : 'text-sm'}`}>
                    <input
                      type="radio"
                      name="municipality"
                      value={mun.name}
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {[2024, 2023, 2022, 2021].map(year => (
                  <label 
                    key={year} 
                    className={`flex items-center space-x-2 border rounded-lg cursor-pointer transition ${isMobile ? 'px-2 py-1 text-xs' : 'px-3 py-2 text-sm'}`}
                    style={{
        ...
        {/* MENSAJE INICIAL */}
        {!selectedMunicipality && (
          <div className={`bg-blue-50 border-l-4 border-blue-600 rounded-lg text-center ${isMobile ? 'p-4' : 'p-6'}`}>
